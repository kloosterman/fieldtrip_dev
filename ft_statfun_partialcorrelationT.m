function [s, cfg] = ft_statfun_partialcorrelationT(cfg, dat, design)
...
%
% NK edit: design can contain extra lines with vars to control for, using
% partialcorr
%
...
if strcmp(cfg.computestat,'yes') % compute the statistic
  ...
  nvars = size(design,1);
  if nvars == 1
    rho = corr(dat', design', 'type', cfg.type); % original
  elseif nvars == size(dat,1) % % we have a var per bin
    rho = nan(nvars-1,1);
    for i = 1:size(dat,1)
      rho(i,:) = partialcorr(dat(i,:)', design(1,:)', design(i+1,:)', 'type', cfg.type);
    end
  else %if nvars == 2 % we have 1 or more extra var to control for
    rho = partialcorr(dat', design(1,:)', design(2:end,:)', 'type', cfg.type); % ori
%   elseif nvars == 3 % we have 2 extra var to control for TODO fix for multiple fixed extra vars
%     rho = partialcorr(dat', design(1,:)', design(2:end,:)', 'type', cfg.type); % ori
  end
  clear dat

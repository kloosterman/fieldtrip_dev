function [lrp] = ft_lateralizedfreq(cfg, avgL, avgR)

% FT_LATERALIZEDPOTENTIAL computes lateralized freq powerspectra
% 
%
% Use as
...
avgL = ft_checkdata(avgL, 'datatype', 'freq');
avgR = ft_checkdata(avgR, 'datatype', 'freq');
...
if ~isequal(avgL.freq, avgR.freq)
  ft_error('the freq axes are not the same');
end

cfg.method        = ft_getopt(cfg, 'method',    'lrp'); % modulationindex
...
if isfield(avgL, 'dimord') && isfield(avgR, 'dimord')
    ...
    ft_error('''dimord'' not found. The function expects freq data');
end
...
%     lrp.labelcmb{end+1}     = sprintf('%s/%s', cfg.channelcmb{i,1}, cfg.channelcmb{i,2});
    lrp.label{end+1} = cfg.channelcmb{i,1};
    erpC3L = avgL.powspctrm(C3L,:,:);
    erpC4L = avgL.powspctrm(C4L,:,:);
    erpC3R = avgR.powspctrm(C3R,:,:);
    erpC4R = avgR.powspctrm(C4R,:,:);
    switch cfg.method
      case 'lrp'
        lrp.powspctrm(end+1,:,:) = 1/2 * ((erpC3R - erpC4R) + (erpC4L - erpC3L)); % avg of c-i right and c-i left
      case 'modulationindex' % difference divided by sum
        lrp.powspctrm(end+1,:,:) = 1/2 * (((erpC3R - erpC4R)./(erpC3R + erpC4R)) + ...
          ((erpC4L - erpC3L)./(erpC4L + erpC3L)));
    end
  end
end
